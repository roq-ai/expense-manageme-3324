...
function BusinessListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface[]>(
    ...
    () =>
      getBusinesses({
        relations: ['user', 'earning.count', 'expenditure.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('earning', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>earning</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('earning', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.earning}</Td>
                    )}
                    {hasAccess('expenditure', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.expenditure}</Td>
                    )}
                    <Td>
